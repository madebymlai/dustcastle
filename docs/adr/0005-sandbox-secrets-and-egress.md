...
## Amendment: Build Egress is one descriptor field, not phase tables (dustcastle-j4g)

The per-Package-Manager Build Egress host now lives on the manager's Ecosystem Registry descriptor as `registryHost` ([ADR 0001](0001-nix-store-as-the-toolchain-mechanism.md)), derived from the closed `PackageManager` union — so it is exhaustive at `tsc` like every other dispatch grain (a half-added manager fails to compile rather than silently reaching no registry via `?? []`). `registryHost` is present **iff** the manager carries an `impuritySignal` — the network half of the same biconditional that governs `impureInstall` (npm/pnpm/bun → `registry.npmjs.org`, yarn → `registry.yarnpkg.com`, pip/uv/poetry → `pypi.org`; go/cargo build pure and name no registry). This closed a latent gap: the old free-`string` egress table omitted `uv` and `poetry`, so an impure uv/poetry build had `pypi.org` blocked.

Earlier (`dustcastle-gy5.4`/`.5`) the egress derivation also carried `lockOnlyResolve` and `cargo-vendor` "phases" — standalone host tables (`LOCK_ONLY_RESOLVE_HOSTS`, `CARGO_VENDOR_HOSTS`) plus a Cargo.lock `source = "git+…"` git-host derivation — intended to allowlist the host-side loose-pin resolve and the Cargo vendor fetch. The **4ky amendment above superseded that design**: the host-side resolve/export is *not* proxy-confined (it runs as a trusted host subprocess under the env floor), and the Cargo vendor fetch is a hash-pinned Nix FOD (network by FOD design). Neither flows through the Sandbox egress proxy, so those phases reached nothing in production. They are removed; `deriveEgress` now scopes the Sandbox proxy's allowlist only — the impure runtime install's registry + the repo's git host. **Do not re-introduce per-phase egress tables** for the resolve or the vendor fetch: by the 4ky decision they are deliberately outside this allowlist.
