...
- **No FOD.** A Package Manager descriptor carries an **install command** (frozen when a lockfile is present — `npm ci`, `--frozen-lockfile`, `uv sync`, `cargo build`; resolving fallback when not) and the **registry host** it fetches from. `src/nix/*` (the dep Importers), `lockOnlyResolve`/pin-then-pure, and the `uv`/`poetry` export front-ends all delete. Nix remains only for **Toolchain** provisioning into the Store (ADR 0001).
