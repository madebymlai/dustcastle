...
- **Miss installs and may populate.** On a miss, the first Sandbox for a new fingerprint still runs the normal in-Sandbox install, then populates the Deps Cache with the assembled stage dir.
...
- **Manifest-keyed invalidation.** Edits to dependency-determining files produce a new fingerprint and therefore a fresh install; unrelated repo edits do not.
