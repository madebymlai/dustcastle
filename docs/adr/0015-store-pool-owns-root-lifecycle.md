...
- **`storePool.pin`/`release` become live and stay cross-process-safe** — the Store's pins are on-disk roots that `nix-store --gc` honours regardless of which process wrote them, so routing through the seam keeps the protection it always had. A run supplies every active Toolchain closure to the Store pool (not just the first detected Ecosystem): a polyglot run pins and warms N deduped Store keys, then releases each scoped root on completion. The earlier single-entry closures map was an under-specification, not a primary-only policy. This does **not** touch the deps-cache *in-memory* pin limitation (dustcastle-xyx).
