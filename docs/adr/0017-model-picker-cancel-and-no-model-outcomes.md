...
accepted — refines [ADR 0009](0009-no-project-local-config.md)'s global pi model picker without changing where the model is stored. [ADR 0019](0019-config-hub-replaces-model-command.md) later moves the explicit re-pick surface from `dustcastle model` to `dustcastle config` while preserving the run-time outcomes below.

## Context

The first `dustcastle run` with no global model and the explicit config-hub model action both use the pi model picker. Before the Terminal-port refactor, Ctrl-C lived inside the select widget and called `process.exit(0)`: an interrupt looked like success and the command layer could not distinguish "operator cancelled" from "pi returned no models".

The picker also has three materially different no-selection cases:

- the operator cancelled an interactive picker;
- the explicit config model action cannot show a picker because the terminal is not interactive;
...
- `dustcastle run` maps first-run picker cancellation to exit 130.
...
- The interactive "pi has no models" path remains distinct from cancellation and keeps the ADR 0009 hint: run `pi`, then `/login`, then re-run `dustcastle config`.
- Headless first-run with no configured model must not silently provision nothing; it prints an actionable `dustcastle config` hint, exits non-zero, and provisions nothing.
