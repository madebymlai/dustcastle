...
//   IN-SANDBOX by a real `npm ci` routed through the standing egress proxy (the npm
//   registry is on the {registry, git, model} allowlist), NOT mounted offline from a
//   deps FOD. This is the Node case of the shared ADR 0012 run harness.
//
...
describe("dustcastle run (slice 2: Node in-Sandbox install, ADR 0002/0005/0008/0012)", () => {
  e2e(
    "installs node_modules in-Sandbox via the egress proxy, then runs `npm test` green",
    async () => {
      ...
      // The deps install IN-SANDBOX via `npm ci`, not a Store copy.
