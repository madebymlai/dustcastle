...
const CASES = [
  { manager: "pnpm", fixture: PNPM_SAMPLE, install: "pnpm install" },
  { manager: "yarn", fixture: YARN_SAMPLE, install: "yarn install" },
] as const;

describe("dustcastle run (slice 2b: pnpm/yarn in-Sandbox install, ADR 0002/0008/0012/0020)", () => {
  for (const { manager, fixture, install } of CASES) {
    e2e(
      `installs node_modules in-Sandbox over normal networking for a ${manager} project, then runs \`node --test\` green`,
      async () => {
