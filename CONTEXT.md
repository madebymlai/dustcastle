...
**Authored Source**:
The project's committed content at git `HEAD` — what the developer authored and committed, never what the install wrote into the worktree. This is the grain for both the Store (which stages via `git archive HEAD`) and the Deps Cache (which fingerprints committed dependency files). Files absent from `HEAD` — an exported `requirements.txt`, a manufactured lockfile, any install artifact — are invisible to both pools. In a project with no `HEAD` (no commits, or not a git repo) the system degrades gracefully to live worktree content.
_Avoid_: committed, HEAD (reserve "Authored Source" for the concept)

**Project Deps**:
The packages declared by a single project's manifest + lockfile, unique to that project and changing whenever the lockfile does. Distinct from the Toolchain: dustcastle installs them by running the repo's own Package Manager (impure — the real install, lifecycle scripts and all), then **caches the assembled result keyed by the Authored Source fingerprint** so a repeat Sandbox restores it instead of re-installing (see ADR 0012/0016). Not built into the Store and not byte-reproducible — dustcastle does not trade on hermetic deps.
_Avoid_: dependencies (ambiguous — always qualify as Project Deps or Toolchain)

**Warm / cold**:
An entry is **warm** when it is resident, so a Sandbox that wants it gets it instantly; it goes **cold** when GC collects it, so the next use pays a re-fetch or re-install. This applies to **both** managed pools — the Store (Toolchain closures) and the deps cache (assembled Project Deps keyed by Authored Source fingerprint) — kept warm by one recency/ceiling brain that lets the rest go cold under disk pressure (see ADR 0007/0012).
...
**Ecosystem Registry**:
The single, closed, internally-curated set of Ecosystem + Package Manager descriptors that the detect/store/sandbox sites all *derive* from, so per-Ecosystem knowledge is owned in one place rather than smeared across dispatch sites. **Internal curation, not a user-facing plugin system** (ADR 0001): closed and vetted, so adding an Ecosystem is dustcastle's deep, local change — the user never configures one. There are no gated managers: a Package Manager dustcastle can't build hermetically is no longer a special state, because every manager installs impurely (ADR 0012).
_Avoid_: catalog, plugin system, provider registry

### Credentials (ADR 0018)

**Credential**:
A recognised host secret — currently a git-host token (GitHub, GitLab) — that dustcastle hands **into** a Sandbox so a build can authenticate, e.g. a private dependency's git clone. Drawn from a **closed, curated catalog** (the Ecosystem Registry's sibling, ADR 0001): dustcastle owns the set of recognised Credentials and, per entry, how it wires itself into git (a git credential helper) — so the operator fills a **value**, never invents a key. Per-user and global like the model selection: stored as a plaintext value in `~/.dustcastle/config.json` and injected into **every** Sandbox (ADR 0018, which for this one explicit channel supersedes ADR 0005's "never a config file / scoped to a build" posture).
_Avoid_: env passthrough, secret, env var (reserve "Credential" for a curated, self-wiring entry)
